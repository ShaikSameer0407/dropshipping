...
//affiliate
app.post('/signup2submit', (req, res) => { 
  const { username, firstname, lastname, email, password } = req.body;
...
//home
app.post('/signup3submit', (req, res) => { 
  const { username, firstname, lastname, email, password } = req.body;
          ...
          .then((result) => {
            if (result) {
              db.collection('Customer').doc(docs.docs[0].id).get()
                .then((doc) => {
                  res.redirect("/drop"); // Redirect to home page
                })
                .catch((error) => {
...
//affiliate login
app.post('/signin2submit', (req, res) => {
  const { email, password } = req.body;
        ...
        bcrypt.compare(password, customerData.password)
          .then((result) => {
            if (result) {
              db.collection('Customer').doc(docs.docs[0].id).get()
                .then((doc) => {
                  res.redirect("/affliate"); // Redirect to home page
                })
                .catch((error) => {
                  console.error("Error retrieving customer data:", error);
                  res.send("Login failed");
...
//home login
app.post('/signin3submit', (req, res) => {
  const { email, password } = req.body;
    ...
    .then((docs) => {
      if (docs.size > 0) {
        ...
        bcrypt.compare(password, customerData.password)
          .then((result) => {
            if (result) {
              db.collection('user').doc(docs.docs[0].id).get()
                .then((doc) => {
